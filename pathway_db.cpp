...
pathway_db::pathway_db(int argc, char * argv[]){
     ...
     if (lfile!="NOCONFIG"){
		  ...
		  fprintf(lfd,"This study includes %d GO categories or pathways, %d genes and %d snps.\n", pathway_names_.size(), gene_num, snp_ids_.size());
...
//	       printf("%d\n", num_gene_pathway[i]);
     ...
     printf("%d significant genes and %d pathways found\n", N,j);
     ...
     vector<vector<int> > random_snp_list;
     ...
     permutation_prepare(permutation_number, N, &sim_matrix, &random_snp_list);
...
*/
int pathway_db::permutation_prepare(int permutation_number, int N, vector<vector<int> > *sim_matrix, vector<vector<int> > *random_snp){
     ...
     for (i=0;i<permutation_number;i++){
		  ...
		  random_snp->push_back(sim_snp_id);
